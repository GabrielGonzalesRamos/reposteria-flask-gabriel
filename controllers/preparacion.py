...
def validarOrden(orden):
    return base_de_datos.session.query(PreparacionModel).filter_by(preparacionOrden = orden).first()
...
class PreparacionesController(Resource):
    ...
    def get(self, postre_id):
        ...
        print(base_de_datos.session.query(PreparacionModel).filter_by( postre=postre_id).with_entities(PreparacionModel.preparacionDescripcion, PreparacionModel.preparacionOrden).all())
        data = base_de_datos.session.query(PreparacionModel).filter_by(postre = postre_id).order_by(PreparacionModel.preparacionOrden.asc()).all()
        if data:
            resultadoGeneral = data[0].preparacionPostre.json()
            resultado = []
            for i in data:
                print(i.preparacionPostre.json())
                resultado.append(i.json())
            ...
            return {
                'success': True,
                'content': None,
                'message': 'El postre aún no  tiene preparaciones'
            }, 200
